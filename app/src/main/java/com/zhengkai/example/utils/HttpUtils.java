package com.zhengkai.example.utils;

import android.util.Log;

import com.android.volley.RequestQueue;
import com.android.volley.Response;
import com.android.volley.VolleyError;
import com.android.volley.toolbox.StringRequest;
import com.android.volley.toolbox.Volley;
import com.google.gson.Gson;
import com.google.gson.JsonArray;
import com.google.gson.JsonParser;
import com.google.gson.reflect.TypeToken;
import com.nostra13.universalimageloader.utils.L;
import com.zhengkai.Bean.ChatMessage;
import com.zhengkai.Bean.JSONBean;
import com.zhengkai.example.widget.Config;
import com.zhengkai.example.widget.Result;
import com.zhengkai.example.widget.SeedURL;

import java.io.ByteArrayOutputStream;
import java.io.IOException;
import java.io.InputStream;
import java.io.UnsupportedEncodingException;
import java.lang.reflect.Type;
import java.net.HttpURLConnection;
import java.net.URL;
import java.net.URLEncoder;
import java.util.ArrayList;
import java.util.Date;
import java.util.LinkedList;
import java.util.Objects;

import okhttp3.Call;
import okhttp3.Callback;
import okhttp3.OkHttpClient;
import okhttp3.Request;

import static android.content.ContentValues.TAG;


/**
 * http工具类
 */
public  class HttpUtils  {



    /**
     * 发送消息到服务器
     *
     *
     * @return：消息对象
     */


    public static ChatMessage sendMessage(  String message) {
        ChatMessage chatMessage = new ChatMessage();


        String gsonResult = doGet(message);

        System.err.println(gsonResult);


        Gson gson = new Gson();
//        获取从服务器接受到的JSON串
        Result result = null;

        if (gsonResult != null) {
            try {
//                解析JSON
                Type listType = new TypeToken<LinkedList<JSONBean>>(){}.getType();

                LinkedList<JSONBean> users = gson.fromJson(gsonResult, listType);
                System.err.println(users.size());

                int i =users.size();

                chatMessage.setMessage(users.get(0).getQuestion());

                switch (i) {
                    case 1:
                        chatMessage.setMessage(users.get(0).getQuestion());
                        chatMessage.setMessage2(users.get(1).getQuestion());
                        break;
                    case 3:
                        chatMessage.setMessage(users.get(0).getQuestion());
                        chatMessage.setMessage2(users.get(1).getQuestion());
                        chatMessage.setMessage3(users.get(2).getQuestion());
                        break;
                    case 4:
                        chatMessage.setMessage(users.get(0).getQuestion());
                        chatMessage.setMessage2(users.get(1).getQuestion());
                        chatMessage.setMessage3(users.get(2).getQuestion());
                        chatMessage.setMessage4(users.get(3).getQuestion());
                        break;
                    case 5:
                        chatMessage.setMessage(users.get(0).getQuestion());
                        chatMessage.setMessage2(users.get(1).getQuestion());
                        chatMessage.setMessage3(users.get(2).getQuestion());
                        chatMessage.setMessage4(users.get(3).getQuestion());
                        chatMessage.setMessage5(users.get(4).getQuestion());
                        break;
                    default:
                        chatMessage.setMessage(users.get(0).getQuestion());
                        chatMessage.setMessage2(users.get(1).getQuestion());
                        chatMessage.setMessage3(users.get(2).getQuestion());
                        chatMessage.setMessage4(users.get(3).getQuestion());
                        chatMessage.setMessage5(users.get(4).getQuestion());

                }

               /* chatMessage.setMessage(users.get(0).getQuestion());
                chatMessage.setMessage2(users.get(1).getQuestion());
                chatMessage.setMessage3(users.get(2).getQuestion());
                chatMessage.setMessage4(users.get(3).getQuestion());
                chatMessage.setMessage5(users.get(4).getQuestion());*/

                chatMessage.setAnswer(users.get(0).getAnswer());
                chatMessage.setAnswer2(users.get(1).getAnswer());
                chatMessage.setAnswer3(users.get(2).getAnswer());
                chatMessage.setAnswer4(users.get(3).getAnswer());
                chatMessage.setAnswer5(users.get(4).getAnswer());


            } catch (Exception e) {

                Log.d(TAG, "错误");

            }
        }else {
            Log.d(TAG, " 恭喜！您发现了一片知识知识的荒原，进入后台piggrush.cn添加您想要的问题和答案，点击“信息检索”拓展知识库！");
        }
        chatMessage.setData(new Date());
        chatMessage.setType(ChatMessage.Type.INCOUNT);


        return chatMessage;
    }


    /**
     * get请求
     *
     * @param message ：发送的话
     * @return：数据
     */
    public static String doGet(String message) {

        String result = "";
        String url = setParmat(message);
        InputStream is = null;
        ByteArrayOutputStream baos = null;
        try {
            URL urls = new URL(url);
            HttpURLConnection connection = (HttpURLConnection) urls
                    .openConnection();
            connection.setReadTimeout(5 * 1000);
            connection.setConnectTimeout(5 * 1000);
            connection.setRequestMethod("GET");

            is = connection.getInputStream();
            baos = new ByteArrayOutputStream();
            int len = -1;
            byte[] buff = new byte[1024];
            while ((len = is.read(buff)) != -1) {
                baos.write(buff, 0, len);
            }
            baos.flush();
            result = new String(baos.toByteArray());
        } catch (Exception e) {
            e.printStackTrace();
        } finally {
            if (is != null) {
                try {
                    is.close();
                } catch (IOException e) {
                    e.printStackTrace();
                }
            }

            if (baos != null) {
                try {
                    baos.close();
                } catch (IOException e) {
                    e.printStackTrace();
                }
            }
        }

//        result是一个完整的JSON串
        return result;

    }


    /**
     * 设置参数
     *
     * @param message : 信息
     * @return ： url
     */
    private static String setParmat(String message) {


        String url = "";
        try {
            url = Config.URL_KEY + URLEncoder.encode(message, "UTF-8").toString();

        } catch (UnsupportedEncodingException e) {
            e.printStackTrace();
        }
        return url;
    }
}